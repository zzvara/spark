...
private[yarn] class YarnAllocator(
  ...
  def handleAllocatedContainers(allocatedContainers: Seq[Container]): Unit = {
    ...
    */
    ...
    logInfo("Received %d containers from YARN (PIQOS), launching executors on %d of them."
      .format(allocatedContainers.size, allocatedContainers.size))
  }
  ...
  private def runAllocatedContainers(containersToUse: Seq[Container]): Unit = {
    for (container <- containersToUse) {
      ...
      if (launchContainers) {
        logInfo("Creating launcher executable.")
